...
export class ActualizarreservaComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private _peticion: RestService, private _peticionNestor: NestorService, public Usuario: DatosUsuario, public confirmacion: MatDialog,
              private toastr: ToastrService, private autenticacion: DatosUsuario){
    this.spinner = true;
    this.visible = false;
      ...
      this._peticion.getselect('reserva/filtrar-rol-usuario?nivel_rol=' + this.Usuario.datosLogin.User.nivel_rol + '&area_rol=' + this.Usuario.datosLogin.User.area_rol).subscribe((respuesta) => {
        ...
        setTimeout(() => {
          if (this.Usuario.datosReserva.length > 0) {
            ...
            this.toastr.info('No hay registros para este usuario', 'Información', { timeOut: 1500 });
            this.visible = false;
          }
          this.spinner = false;
        }, 600);
  ...
  mostrarReserva(id: number): void {
    ...
    setTimeout(() => {
      this._peticion.getunidad('reserva/' + id).subscribe((respuesta) => {
        ...
        this.crearreserva.controls['horafin'].setValue(this.horafinal[0].value);
        if (this.Usuario.datosReserva.id_usuario_colaborador === 0) {
          this.crearreserva.controls['tiporeserva'].setValue(this.cbtiporeserva[1].value);
        } else {
          this.crearreserva.controls['tiporeserva'].setValue(this.cbtiporeserva[0].value);
          setTimeout(() => {
            this.crearreserva.controls['encargado'].setValue(String(this.Usuario.datosReserva.id_usuario_colaborador));
          }, 1500);
        }
        this._peticion.getunidad('unidadorganizacional/' + this.Usuario.datosReserva.id_unidad_organizacional).subscribe((respuesta) => {
          this.crearreserva.controls['tipo'].setValue(respuesta.id_tipo_espacio);
          this.crearreserva.controls['sede'].setValue(respuesta.id_unidad_organizacional_padre);
          setTimeout(() => {
            if (this.Usuario.datosReserva.id_usuario_colaborador != 0) {
              setTimeout(() => {
                this.crearreserva.controls['codigo'].setValue(this.Usuario.datosReserva.codigo_programa);
                setTimeout(() => {
                  this.crearreserva.controls['nivel'].setValue(this.Usuario.datosReserva.nivel);
                  setTimeout(() => {
                    this.crearreserva.controls['grupo'].setValue(this.Usuario.datosReserva.nombre_grupo);
                    setTimeout(() => {
                      this.crearreserva.controls['submodulo'].setValue(this.Usuario.datosReserva.submodulo);
                    },500);
                  },500);
                },500);
              },250);
        ...
        let arrDays: string[] = [];
        for ( let item of this.Usuario.datosReserva.reservaDias) {
          if (arrDays.indexOf(item.reserva_dia_dia) === -1) {
            arrDays.push(item.reserva_dia_dia)
          }
        }
        this.crearreserva.controls['dia'].setValue(arrDays);
        this.toastr.warning('Verifique la hora de fin', 'Alerta', { timeOut: 3500 });
      });
    }, 200);
  }
    ...
    dialogRef.afterClosed().subscribe(res => {
      if (res) {
        this._peticion.delete('reserva/' + id).subscribe((respuesta) => {
          if (respuesta.message === "Sequence contains no elements.") {
            this.toastr.success('Registro eliminado con exito', 'Exitoso', { timeOut: 1500 });
          } else {
      ...
      if (validarRepeticion) {
        ...
        this._peticion.postreserva('reserva/obtener-espacios-disponibles', this.objetoreserva).subscribe((respuesta) => {
          if (respuesta.message === 'No hay espacios disponibles') {
            ...
            this.listaUnidades = [];
          } else if (respuesta.message === 'No se pudo obtener espacios') {
  ...
  actualizarReserva() {
    ...
    if (!this.validarForm) {
      ...
      if (validarRepeticion) {
        ...
        this._peticion.update('reserva', this.objetoreserva).subscribe((respuesta) => {
          this.crearreserva.controls['disponibles'].setValue(0);
          this.listaUnidades = null;
          setTimeout(() => {
  ...
  validarFechaDia(inicial: number, final: number): boolean {
    let fechas = [];
    for (let i = inicial; i < 8; i++) {
      if (i === 7) {
        i = 0;
      }
      fechas.push(i);
    ...
    let dias = [];
    for (let i = 0; i < 7; i++) {
      if (this.crearreserva.value.dia[i] === 'Lunes') {
        dias.push(0);
      } else if (this.crearreserva.value.dia[i] === 'Martes') {
        dias.push(1);
      } else if (this.crearreserva.value.dia[i] === 'Miércoles') {
        dias.push(2);
      } else if (this.crearreserva.value.dia[i] === 'Jueves') {
        dias.push(3);
      } else if (this.crearreserva.value.dia[i] === 'Viernes') {
        dias.push(4);
      } else if (this.crearreserva.value.dia[i] === 'Sábado') {
        dias.push(5);
      } else if (this.crearreserva.value.dia[i] === 'Domingo') {
        dias.push(6);
      }
    }
    let cnt = 0;
